...
//        of the array in order of their appearance in the array. Previous 
...
int main() {
    ...
    // For each element, find the nearest greater element on the left
    for (int i = 0; i < n; i++) {
        int prevGreater = -1;
        ...
        for (int j = i - 1; j >= 0; j--) {
            if (arr[j] > arr[i]) {
                prevGreater = arr[j];
                break; // first greater from left
            }
        }

        // Print result with comma format
